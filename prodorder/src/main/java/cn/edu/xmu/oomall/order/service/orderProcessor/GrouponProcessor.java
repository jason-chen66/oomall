...
@Service("GrouponProcessor")
public class GrouponProcessor implements OrderProcessor {
    private RocketMQTemplate rocketMQTemplate;
    @Autowired
    public GrouponProcessor(RocketMQTemplate rocketMQTemplate){
    ...
    public void orderPayStatus(Order order)throws BusinessException {
        if(order.getStatus().equals(Order.NEW)){
            order.setStatus(Order.NOT_GROUPON);
        }
